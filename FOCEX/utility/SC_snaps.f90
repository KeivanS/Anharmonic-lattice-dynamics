...
!========================
module ios
integer, parameter:: ulog=9,usnap=8,umode=7
end module ios
!========================
module pairpot
...
!! this program generates a surpercell and snapshots of it from the canonical ensemble where
...
!! INPUT FILES: cell.inp for the primitive cell; format is similar to structure.params
!!              supercell.inp contains the supercell transl vect in terms of the primitiv
!!              snaps.inp contains the average frequency, the temperature and the number of needed snapshots
use cells
use ios
use pairpot
implicit none
integer i,i0,j,j1,l,s,s2,k, mode,nmodes,cnt
real(8), allocatable:: phi(:),ksi(:),normal_modes(:,:),freq_thz(:),disp(:,:),gaussian(:)
real(8) tempk,wavgcm,den
...
character(3) csn
   ...
   if (cell.eq.0) then ! generate displacements according to primcell modes (k=0)
      allocate(freq_thz(3*natoms0),normal_modes(3*natoms0,3*natoms0))
      call get_normal_modes(natoms0,pos_prim,mass,normal_modes,freq_thz,wavgcm)
      nrand = 3*natoms0*nsnap*n_sc
   else ! generate exactly nmode-3 displacements according to the eigenvectors of the supercell
...
!   write(*,*)' Normal modes calculated and written'

! if snapshots are displacements along normal modes
   allocate(ksi(nrand),phi(nrand),seed(nrand),disp(natom_sc,3)) !random numbers for each mode
   allocate(gaussian(nrand)) !modes))
...
!  write(*,*) ' generating ',nrand,' random number of gaussian distribution'
   ...
   do i=1,nrand
      ...
      write(50,4) ' ',gaussian(i)
   enddo
   close(50)
   write(*,*)nrand,' Gaussian Random numbers generated and written in gauss_0_1.dat'
...
! writing first (unmoved) snapshot
   ...
   write(csn,'(i3.3)')s
   open(69,file='POSCAR')
   write(69,*)' undistorted supercell '
   write(69,*)' 1 '
   write(69,3) rsc1 
   write(69,3) rsc2 
   write(69,3) rsc3 
   write(69,*)  (n_sc*natom0(j),j=1,ntype) !natom_sc
   write(69,*)' C '
   ...
   do i=1,natom_sc
      ...
      write(70+s ,4) atname_sc(i),pos_sc(i,:)
   enddo
   close(69)

   open(60,file='sqdisp.dat')

       if(cell.eq.0 .and. natoms0.ne.1 ) then  ! normal modes of prim cell with k=0

         snaploop0: do s=1,nsnap   ! the required # of snapshots
!          write(usnap,*)'###############'
           write(csn,'(i3.3)')s
           open(70+s,file='snap_'//csn//'.xyz')
           write(usnap,*) natom_sc
           write(usnap,*)'# Snapshot ',s
           s2=(s-1)*3*natoms0*n_sc   ! starting counter for each snapshot

           disp=0;den=0
           do k=1,n_sc  ! loop over different prim cells in the SC
           do i0=1,natoms0
              do j=1,3
                s2=s2+1  
                i=ipos(i0,k)
                do mode=4,3*natoms0  ! move each primcell along one mode randomly
!                  call random_number(mode_ran)
                   disp(i,j)=disp(i,j)+normal_modes(3*(i0-1)+j,mode)  &
        &          * gaussian(s2)*sqrt(tempk/mass(i0))/freq_thz(mode)

                enddo
              enddo
           enddo
           enddo
           den=sum(gaussian((s-1)*3*natoms0*n_sc+1:(s)*3*natoms0*n_sc))
           disp=1e10*sqrt(1.38e-23/1.67e-27)/1e12/6.28318531*disp /den !=0.14528*disp ; to convert to Ang

!          write(60,*)'# snap ',s
           do i=1,natom_sc
              do j=1,3
                 write(60,3) disp(i,j) !sqrt(disp(i,1)**2+disp(i,2)**2+disp(i,3)**2)
              enddo
              write(usnap,4) atname_sc(i),pos_sc(i,:)+disp(i,:)
              write(70+s ,4) atname_sc(i),pos_sc(i,:)+disp(i,:)
           enddo
           close(70+s)

         enddo snaploop0

       else ! use eigenmodes of the supercell
...
!#      write(usnap,*)'###############'
   ...
   enddo
           ...
           disp=0
           den=0
           do mode=4,nmodes  ! sum excludes acoustic modes, which are pure translation for k=0
           do i=1,natom_sc
           do j=1,3
              disp(i,j)=disp(i,j)+normal_modes(3*(i-1)+j,mode)  &
        &     * gaussian(mode-3)*sqrt(tempk/mass_sc(i))/freq_thz(mode)
           enddo
           enddo
              den=den+gaussian(mode-3)
           enddo
           disp=1e10*sqrt(1.38e-23/1.66e-27)/1e12/6.28318531*disp /den !=0.14528*disp
...
!          write(60,*)'# snap ',s
   ...
   write(*,*)nsnap,' snapshots generated and written in snapshots.xyz and snap_i.xyz'
   deallocate(ksi,phi,gaussian,seed,disp)

   close(usnap)
   close(ulog)

3 format(9(1x,f15.8))
4 format(a,2x,3(1x,f15.8))
...
allocate(pos_conv_red(natoms0,3),pos_prim(natoms0,3),mass(natoms0),atname(natoms0))
write(ulog,*)' Cartesian coordinates of the primitive atoms '
k=0
do j=1,ntype
   do i=1,natom0(j)
      k=k+1
      read(10,*)atname(k), pos_conv_red(k,:)! this pos is in units of conventional vectors; convert to cartesian
      mass(k)=ms(j)
      pos_prim(k,:)=pos_conv_red(k,1)*r1+pos_conv_red(k,2)*r2+pos_conv_red(k,3)*r3
      write(ulog,3)pos_prim(k,:) ! converted to Cartesian coordinates of the primitive atoms 
   enddo
enddo
...
use pairpot
implicit none
integer n1(3),n2(3),n3(3),i,j,k,i1,i2,i3,l1,l2
real(8) r23(3),r_sc,rr(3),x,y,z,eps,rij

open(10,file='supercell.inp',status='old')
read(10,*) n1   !  in terms of conventional translation vectors
read(10,*) n2
read(10,*) n3
close(10)
...
! b3=b3/vol_sc
 ...
 allocate(pos_sc(natom_sc,3),mass_sc(natom_sc),atname_sc(natom_sc),ipos(natoms0,n_sc))
...
do l2=1,natom0(l1)
   i=i+1
   k=0
   do i1=-20,20  ! sum over other primitive cells
   do i2=-20,20
   do i3=-20,20
      rr=i1*r01+i2*r02+i3*r03
...
! check to see if rr is inside the supercell
      ...
      j=j+1
      ipos(i,k)=j
      pos_sc(j,:)=pos_prim(i,:)+rr(:)
      mass_sc(j)=mass(i)
      atname_sc(j)=atname(i)
      write(22,4) atname_sc(j),pos_sc(j,:) !,x,y,z,rr
   enddo
   enddo
   enddo
enddo
...
real(8), intent(in ):: ps(natom,3),mas(natom),wavgcm
...
!! the strength of the FCs are fixed so that the average frequency squared is wavgcm^2 in 1/cm
 write(*,*)' Now calculating normal modes with a Morse potential'
 it_max=100
...
! impose periodic BCs
   ...
   if(rij.lt.rcut) then
     ...
     enddo
     write(*,19)i,j, fc(3*(i-1)+al,3*(j-1)+1),fc(3*(i-1)+al,3*(j-1)+2),fc(3*(i-1)+al,3*(j-1)+3)
     enddo
   endif
...
! apply ASR
    ...
    enddo
    close(90)
    write(*,*)'Force constants calculated and written in fcs.dat'
...
! call EIGCH(cfc,3*natom,3*natom,3*natom,3*natom,1d-8, eival, eivec, IER)
 ...
 if (it_num+1.eq.it_max) then !ier.ne.0) then
    ...
    write(umode,*)"# UNSCALED EIGENVALUES (raw,THz) & EIGENVECTORS OF THE FC MATRIX after ",rot_num,it_num
    sumw2=0
    do i=1,3*natom
...
! normalize so that average eival is Wavg^2
 ...
 write(umode,*) ' eivals,freqs in THz after renormalization ',wavgcm/33,wavgcm
 do i=1,3*natom
    write(umode,*)i,eival(i),sqrt(abs(eival(i)))*cnst/33
 enddo
 write(*,*)' Eigenvalues and eigenmodes calculated and written in modes.dat'
