...
Now, put the ``POSCAR1``, ``FORCEDISP1`` , ``structure.params``, ``dielectric.params`` and ``kpbs.params`` in same directory, simply run ``./focex.x`` within that directory to run FOCEX. After successful
