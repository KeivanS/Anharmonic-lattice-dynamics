...
module born
   real(8) epsil(3,3)
   real(8), allocatable:: zeu(:,:,:),force_born(:,:,:)
   real(8) rho
   integer born_flag
end module born
!**********************************************************
...
!   allocate(om(mesh),dos(0:la,mesh))
   ...
   IF(.NOT.ALLOCATED(dos)) ALLOCATE(dos(0:la,mesh))
...
subroutine write_dos
   ...
   sumdos=0
   do i=1,wmesh
      sumdos=sumdos+dos(0,i)*wmax/wmesh *cnst
!      write(udos,3)om(i),i,sumdos,(dos(la,i),la=0,min(10,ndyn2))
      write(udos,4) om(i),',',sumdos,',',dos(0,i)
   enddo
   sumdos=sumdos-dos(0,1)*wmax/wmesh/2d0 *cnst

3  format(g11.5,2x,i5,99(1x,g10.4))
4  format(g11.5,2(2x,a,2x,g10.4))
...
subroutine allocate_eig(nb,ni,nk) ! nb for band, nk for coarse mesh in FBZ
   ...
   IF(.NOT.ALLOCATED(eigenval)) ALLOCATE(eigenval(nb,nk))
   IF(.NOT.ALLOCATED(eigenvec)) ALLOCATE(eigenvec(nb,nb,nk))
   IF(.NOT.ALLOCATED(veloc)) ALLOCATE(veloc(3,nb,nk))
   IF(.NOT.ALLOCATED(grun)) ALLOCATE(grun(nb,nk))

   IF(.NOT.ALLOCATED(eivalibz)) ALLOCATE(eivalibz(nb,ni))
   IF(.NOT.ALLOCATED(eivecibz)) ALLOCATE(eivecibz(nb,nb,ni))
   IF(.NOT.ALLOCATED(velocibz)) ALLOCATE(velocibz(3,nb,ni))
...
subroutine allocate_eig_bs(nb,nk,nv) ! nb for band, nk for band structure mesh
   ...
   if (nv.ne.0) then
      ...
      IF(ALLOCATED(eigenvec_bs)) DEALLOCATE(eigenvec_bs)
      IF(ALLOCATED(grun_bs)) DEALLOCATE(grun_bs)
...
! includes firstpoint not the last, except for the last direction
   read(uio,*) ndir  ! number of directions for the band structure
   nkp_bs = ndir*nkdir    ! +1 is for including the last point
   if(.not.ALLOCATED(kp_bs)) allocate(kp_bs(3,nkp_bs))
   if(.not.ALLOCATED(dk_bs)) allocate(dk_bs(nkp_bs))
...
!   ax = boxx; ay=boxy; az=boxz
   lattice_parameter=1d0 !what's the value?
   ax = lattice_parameter
...
! Where is g1,g2,g3 initialized???
   gmaxx = max(abs(g1%component(1)),abs(g2%component(1)),abs(g3%component(1)))
   gmaxy = max(abs(g1%component(2)),abs(g2%component(2)),abs(g3%component(2)))
   gmaxz = max(abs(g1%component(3)),abs(g2%component(3)),abs(g3%component(3)))
...
! shift by a small amount so that only one boundary K remains in the FBZ after folding
   ...
   do j = -ny,ny
   do k = -nz,nz
      nk = nk+1       !shftx/shfty/shftz are not initialized anywhere
...
!WRITE(*,*) 'For CHECK purpose:',g1,g2,g3
!   allocate(mapib(nbz))
...
! n1,n2,n3 loops
 ...
 IF(ALLOCATED(mapibz)) DEALLOCATE(mapibz)
 IF(ALLOCATED(mapinv)) DEALLOCATE(mapinv)
...
! redefine the mapibz now that all ibz kpoints have been identified
  ploop: do i=1,nk
    call getstar(kp(:,i),primitivelattice,narms,kvecstar,kvecop)
    do l=1,narms
       do j=1,nibz
          qaux= (kibz(:,j) - kvecstar(:,l))
          st(1)=(qaux(:) .dot. r1) /2/pi
          st(2)=(qaux(:) .dot. r2) /2/pi
          st(3)=(qaux(:) .dot. r3) /2/pi
          qaux=0
