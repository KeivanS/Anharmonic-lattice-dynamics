...
!          ixyztermzero
      ...
      integer iatomterm(maxrank,maxterms),jterm
      integer icell(3),iv(3),isg,ixyzd(nrank),ntermsindep,msave,nn
      integer irank,k2,neqs,ifactorial,msave2,j2
      integer npermute,ixyzfirst(nrank),nterm5,ixyz4(nrank)
      integer mapdep(maxterms),mapterms(maxterms)
      integer maptermsindep(maxterms),mapindepterms(maxterms)
      integer iatomtermall(maxrank,maxterms),ixyztermall(maxrank,maxterms)
      integer ixyzterm(maxrank,maxterms)
      integer iatomtermindep(maxrank,maxterms),ixyztermindep(maxrank,maxterms)
      integer ntermszero,iatomtermzero(maxrank,maxtermszero),ixyztermzero(maxrank,maxtermszero)


      logical foundit(maxterms),firstone,zero(maxterms)
      double precision v(3),amp, amat(maxterms,maxtermsindep)
...
!    nout=0
!    do j=1,nrow
!		do while(j+nout.le.ncol)
!			!	PUT NON-ZERO ELEMENTS ON DIAGONAL
!			if(ncmp(zna(j,j+nout)).ne.0) then
!				!	NORMALIZE ELEMENTS IN ROW
!                zntemp=zna(j,j+nout)
!                do k=j+nout,ncol
!                	zna(j,k)=zna(j,k)/zntemp
...
!                    zna(k,j+nout)=0.
!                enddo
!                exit !terminate while loop, j++
!			endif
!			do  k=j+1,nrow
!            	if(ncmp(zna(k,j+nout)).ne.0) then
!                	!	INTERCHANGE ROWS
!                    do l=1,ncol
!                        zntemp=zna(j,l)
!                        zna(j,l)=zna(k,l)
...
!                    zntemp=zna(j,j+nout)
!                    do k2=j+nout,ncol
!                    	zna(j,k2)=zna(j,k2)/zntemp
!                    enddo
!                    !	REDUCE ALL OTHER ROWS
!                    do  k2=1,nrow
!                        if(k2.eq.j) cycle
!                        if(ncmp(zna(k2,j+nout)).eq.0) cycle
!                    	do  l=1,ncol
!                            if(l.eq.j+nout) cycle
!                            zna(k2,l)=zna(k2,l)-zna(j,l)*zna(k2,j+nout)
!                    	enddo
!                        zna(k2,j+nout)=0.
!                    enddo
!                    exit !terminate while loop, j++
