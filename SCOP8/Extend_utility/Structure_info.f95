...
!!This module is used for reading info from lat_fc.dat and store those info
    ...
    INTEGER ::nouse1, nouse2, nouse3 !for no use lines in 'structure.params'
...
!    open(unit_params,file='params.inp')
    open(unit_params,file='structure.params')
...
! read(uparams,*) junk
! read(uparams,*) junk
    write(unit_params,*) nouse1, nouse2
...
!-----------------------------------------------------------------------------------------------
    ...
    !! major
        ...
        !CHARACTER(LEN=2) :: name
        INTEGER :: unit_number,unit_log,i,j,direction, maxshell
        REAL(8) :: check(d)
        TYPE(vector) :: vv
        LOGICAL :: condition
...
!#################check further compatibility with input file "params.inp" ##############
        ...
        WRITE(unit_log,*) line
         ...
         READ(unit_number,*) maxshell, tot_atom_number !not the same with OLD!
