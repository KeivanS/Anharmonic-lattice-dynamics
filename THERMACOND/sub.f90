...
!***           kappa_RTA(ii,j,jj)=kappa_RTA(ii,j,jj) - const1 * omega * velocc(j) * nbb * (nbb+1) * FRTA(jj)
          ...
          enddo beta_loop
            ...
            !     tauinv_eff(i,ii)= tauinv_eff(i,ii)-const2*F12(j) / omega / 0.01
           !else
                 tauinv_eff(i,ii)= tauinv_eff(i,ii)-const2* dot_product(F12,velocc) / omega /dot_product(velocc,velocc)  /(narms*3d0)
           !end if
...
nkF1_loop: do i=1,nibz
    laF1_loop: do ii=1,ndn
          ...
          nb0=dist(mapinv(i),ii)
           ...
           write(uRTA,10) tempk,i,kibz(:,i),ii,omega,nb0,1/tauinv_N(i,ii),1/tauinv_U(i,ii),1/tauinv_tot(i,ii),tauinv_eff(i,ii),veloc(:,ii,mapinv(i))!,(kappa_k(i,ii,j,j),j=1,3)
...
!===========================================================
subroutine update_mpi(ndn,ibz_subset)!!Saf!!
!subroutine cal_F2m23(nk,kp,nibbz,kibbz,ndn,tempk) !!Saf!!
! calculate F value based on F value from previous iteration.
! This subroutine uses symmetry operations to calculate F in FBZ through F in IBZ.
! Summation over k1 and k2 in IBZ, but in the case of k2, F(k2 in FBZ) = S*F(k in IBZ)
...
integer ibz_subset(2), k_shift, ksubsize3, ibz
...
!loop1 : do n1=1,nibz
loop1 : do ibz=1,ksubsize3 !ibz_subset(1),ibz_subset(2)
            n1 = ibz+k_shift
           q1=kibz(:,n1)
...
!changetoIBZ       Avalue(mapinv(n1),l1,xyz)=Avalue(mapinv(n1),l1,xyz)+P1(mapinv(n1),ns,l1,l2,l3)*(Fsp1(xyz)-Fsq2(xyz))+0.5*P2(mapinv(n1),ns,l1,l2,l3)*(Fsq2(xyz)+Fsp2(xyz))
                   Avalue(ibz,l1,xyz)=Avalue(ibz,l1,xyz)+P1(ibz,ns,l1,l2,l3)*(Fsp1(xyz)-Fsq2(xyz))+0.5*P2(ibz,ns,l1,l2,l3)*(Fsq2(xyz)+Fsp2(xyz))
                  enddo
                 enddo
           enddo !loop s(narms)
        enddo loop2
...
!changetoIBZ   F2(mapinv(n1),l1,xyz)=F_RTA(mapinv(n1),l1,xyz) + Avalue(mapinv(n1),l1,xyz)/Qvalue(mapinv(n1),l1)
               F2mpi(ibz,l1,xyz)=F_RTA(n1,l1,xyz) + Avalue(ibz,l1,xyz)/Qvalue(n1,l1)
...
real(8) q(3),q1(3),q2(3),q3_p1(3),q3_p2(3)
integer i3,j3,k3,inside,nk3_p1,nk3_p2
real(8) C_d, C_of, C
real(8) kvecstars(3,48)
integer kvecops(48),n0,n22,nl2,l3,nl3,ni0,n1i,d1,d2
...
! K1              C = C + (-P1(n1i,n0,l1,l2,nl3) + P1(n1i,nk3_p1,l1,l2,nl3) +  P2(n1i,n0,l1,l2,nl3))
           !       C = C + (-P1(n1 ,n0,l1,l2,nl3) + P1(n1 ,nk3_p2,l1,l2,nl3) + P2(n1 ,n0,l1,l2,nl3))
                  C = C + (-P1(n1,n0,l1,l2,nl3) + P1(n1,nk3_p2,l1,l2,nl3) + 0.5*P2(n1,n0,l1,l2,nl3) + 0.5*P2(n1,nk3_p1,l1,l2,nl3)) 
                enddo loopnl3
         !    C4 = C_of
         !    C5 = C_of
           endif
...
!===========================================================
 !===========================================================
 subroutine calculate_v35_selftetra_qzero(qe0,ndn,nib,eivlibz,eivcibz,nk,eival,eivec)
! this version is used when direct access is needed to v33 for given k_indices and bands
! this subroutine computes the sum_R2,R3,Tau1,Tau2,Tau3,al,be,ga of  (hbar/2)**1.5
! Psi_R2,R3,Tau1,Tau2,Tau3^al,be,ga e^{i k2.R2+i k3.R3} /sqrt(m_tau1*m_tau2*m_tau3) *
! e_la1^{tau1,al}(k1)*e_la2^{tau2,be}(k2)*e_la3^{tau3,ga}(k3)/sqrt(w_la1(k1)*w_la2(k2)*w_la3(k3))
...
! use force_constants_module
 ...
 use constants
 implicit none
 integer, intent(in) :: ndn,nk,nib !ksubibz(2),ndn,nk,nib
 integer l1,l2,l3,n2,n3,al,be,ga,i0,j0,k0,j,k,t,ta1,ta2,ta3
 integer i1,i2,i3,j1,j2,j3,k1,k2,k3,inside
 integer nk1,nk2,nk3,np1,np2,np3,n1 ,nibzsub
 real(8) q1(3),q2(3),q3(3),mi,mj,mk,rx2(3),rx3(3),den,eivlibz(ndn),eival(ndn,nk),qe0(3)   !eivlibz(ndn,nib),eival(ndn,nk)
 complex(8) eivec(ndn,ndn,nk),xx,eiqr,eivcibz(ndn,ndn)  !eivcibz(ndn,ndn,nib)
 character(4) cibz1,cibz2
 character(6) cnk
 character(132) filname
...
! not initializing might be better if some array elements are not properly initialized
 ...
 !loop1: do n1=ksubibz(1),ksubibz(2)
    n1=1
    np1=1 !n1-ksubibz(1)+1
    q1 = qe0 !kibz(:,n1) ! =kpc(:,mapinv(n1))
    !call get_k_info_cent(q1,NC,nk1,i2,j2,k2,g1,g2,g3,inside)
    call get_k_info(q1,NC,nk1,i2,j2,k2,g1,g2,g3,inside)
    !!if(mapinv(n1).ne.nk1) then
    !!  write(ulog,*)'mapinv(n1),nk1,inside=',mapinv(n1),nk1,inside
...
!&      exp(ci*(kp(:,n2) .dot. atompos(:,j) + kp(:,n3) .dot. atompos(:,k))) /  &
       ...
       xx = xx + fcs_3(t)  /sqrt(mi*mj*mk) * eiqr * &
!&      eivec(ta1,l1,nk1)*eivec(ta2,l2,nk2)*eivec(ta3,l3,nk3)
&     eivcibz(ta1,l1)*eivec(ta2,l2,nk2)*eivec(ta3,l3,nk3)        !   eivcibz(ta1,l1,n1)*eivec(ta2,l2,nk2)*eivec(ta3,l3,nk3)
! make sure for kp at the FBZ boundary it gives (-1) or 1
...
!   den = sqrt(sqrt(abs(eival(l1,nk1)*eival(l2,nk2)*eival(l3,nk3))))
    den = sqrt(eivlibz(l1)*eival(l2,nk2)*eival(l3,nk3))/cnst/sqrt(cnst)   !sqrt(eivlibz(l1,n1)*eival(l2,nk2)*eival(l3,nk3))/cnst/sqrt(cnst)
    if (den.ne.0) then
       xx = xx / den/2d0/sqrt(2d0) * const33
!      write(*,6)'nk_i,la_i,xx=',nk1,nk2,nk3,l1,l2,l3,xx
    else   ! usually acoustic eigenvalues at k=0 are not strictly zero, but a small number
...
6 format(a,6(i5),9(2x,g14.8))
 ...
 end subroutine calculate_v35_selftetra_qzero
!===========================================================
!!===========================================================
 subroutine calculate_v35_selftetra_qzerompi(kfsubset,qe0,ndn,nib,eivlibz,eivcibz,nk,eival,eivec)
! this version is used when direct access is needed to v33 for given k_indices and bands
! this subroutine computes the sum_R2,R3,Tau1,Tau2,Tau3,al,be,ga of  (hbar/2)**1.5
! Psi_R2,R3,Tau1,Tau2,Tau3^al,be,ga e^{i k2.R2+i k3.R3} /sqrt(m_tau1*m_tau2*m_tau3) *
! e_la1^{tau1,al}(k1)*e_la2^{tau2,be}(k2)*e_la3^{tau3,ga}(k3)/sqrt(w_la1(k1)*w_la2(k2)*w_la3(k3))
...
! use force_constants_module
 ...
 character(6) cnk
 character(132) filname
 integer  kf_shift,kfsubsetsize3,kfsubset(2)
...
kfsubsetsize3=(kfsubset(2)-kfsubset(1))+1
kf_shift=kfsubset(1)-1

 loop2: do n2=1,kfsubsetsize3 !nkc   ! second argument k2 needs to be only in the IFBZ
    q2 = kpc(:,n2+kf_shift)
    call get_k_info(q2-shft,NC,nk2,i2,j2,k2,g1,g2,g3,inside)
 !   if(n2.ne.nk2) then
 !     write(ulog,*)'n2,nk2,inside=',n2,nk2,inside
...
!      write(*,6)'nk_i,la_i,xx=',nk1,nk2,nk3,l1,l2,l3,xx
    ...
    !v3sq(np1,nk2,l1,l2,l3)=xx*conjg(xx)
    v3sqmpi_em(np1,n2,l1,l2,l3)=xx*conjg(xx)
 !   write(uv3)n1,nk2,l1,l2,l3,v3sq(np1,nk2,l1,l2,l3)
...
6 format(a,6(i5),9(2x,g14.8))
 ...
 end subroutine calculate_v35_selftetra_qzerompi
!===========================================================
!============================================================
...
!    write(*,*)'v3sq(nqs,nk2,la,l2,l3)',v3sq(nqs,nk2,la,l2,l3),funcp(nk2),funcm(nk2)
    enddo
...
!        uself = uself + cmplx(0,res)
       ...
       !write(*,*)'insde-self-tetra',nself,uself
    enddo
...
!=========================================
 subroutine kk_im2re(om,n,img,x,re)
! computes the real part of a function of om, given its imaginary part
! according to : re(om)=2/pi*\int_0^inf imag(x)*x*/(x*x-om*om) dx
! which is: re(om)=2/pi*\int_0^inf imag(x)*x*(x*x-om*om)/((x*x-om*om)**2+eta**2)*dx
! also: re(om)=1/pi*\int_0^inf imag(x)*((x-om)/((x-om)**2+eta**2)+1/(x+om))*dx
! uses a trapezoidal method and works even on a non-uniform mesh x(i)
! so here x or om is the square of the phonon frequency
 use params
 use constants, only : pi

 implicit none
 integer n,i
 real(8), intent(in):: img(n),om,x(n)
 ...
 do i=1,n-1
    dx=x(i+1)-x(i)
    fm=img(i  )*((x(i  )-om)/((x(i  )-om)**2+et*et)+1/(x(i  )+om))
    fp=img(i+1)*((x(i+1)-om)/((x(i+1)-om)**2+et*et)+1/(x(i+1)+om))
    re=re+(fp+fm)*dx/2
 enddo
 re=re/pi
...
!=========================================
 subroutine kk_re2im(om,n,re,x,imag)
! computes the imaginary part of a function of om, given its real part
! according to : Im(om)=-2*om/pi*\int_0^inf real(x)/(x*x-om*om) dx
! which is: Im(om)=-1/pi*\int_0^inf real(x)(1/(x-om) -1/(x+om)) dx
! also: re(om)=-1/pi*\int_0^inf real(x)*((x-om)/((x-om)**2+eta**2)-1/(x+om))*dx
! uses a trapezoidal method and works even on a non-uniform mesh x(i)
 use params
 use constants, only : pi
 implicit none
 integer n,i
 real(8), intent(in):: re(n),om,x(n)
 real(8), intent(out):: imag
 real(8) dx,fp,fm,et
 ...
 do i=1,n-1
    dx=x(i+1)-x(i)
    fm=re(i  )*((x(i  )-om)/((x(i  )-om)**2+et*et)-1/(x(i  )+om))
    fp=re(i+1)*((x(i+1)-om)/((x(i+1)-om)**2+et*et)-1/(x(i+1)+om))
    imag=imag-(fp+fm)*dx/2
 enddo
 imag=imag/pi
