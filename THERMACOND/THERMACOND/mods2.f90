!==========================================================
 module io2
 integer, parameter:: ulog=30,ubs=50,uparams=13,udos=40,ueiv=60,ukap=70,uklem=71,uslf=3000, &
& ufc0=20,ufc1=21,ufc2=22,ufc3=23,ufc4=24,utimes=11,ualph=27,uvel=31,ufine=67,umod=2000,  &
& fbz=93,bzf=95,ibz=94,ibs=96,ucors=65,uv3=85,ugrun=80,urate=88,ucof=87,ucross=89, k_kappa=9000 
...
!==========================================================
 ...
 real(8), allocatable :: dos(:,:),om(:)
   ...
   enddo
   sumdos=sumdos-dos(ndyn2+1,1)*wmax/wmesh/2d0
...
!==========================================================
 ...
 integer nv3,readv3,writev3, iter, split, calk, max_iter ! sy added iter,split, calk, max_iter
 real(8) v3_threshold,const33, conv_error , conv_max_error, conv_max_diff, conv_diff, conv_diff_kap, conv_max_diff_kap,conv_iter,update   ! sy added conv_norm, conv_max, update
 integer ksub_size
 character(99) v3path
 ...
 contains
  ...
  subroutine allocate_v33(n)
    allocate(v33(n),nq1(n),nq2(n),nq3(n),la1(n),la2(n),la3(n) )
  end subroutine allocate_v33
  subroutine deallocate_v33
    deallocate(v33,nq1,nq2,nq3,la1,la2,la3)
...
! one fine grid in the irreducible FBZ for kappa/dos sums: kibz(3,nibz)
 ...
 real(8) shftx,shfty,shftz,deltak,kcutnbf,kcutnbc   ! kcutnb for nbrlst and interpolation
