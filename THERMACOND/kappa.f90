...
!----------The anharmonic V3 matrix calculation in IBZ-------------------------------------
  nv3 = nkc*nibz*ndyn**3
!-------------------------------------------------------
if ( job .eq. 1 ) then   ! v33 in IBZ MPI...
!-------------------------------------------------------
...
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
 ...
 !write(*,*) displs_mpi,aksub_mpi
  ...
  allocate(P1sm_temp_mpi(ksub_mpi*nkc*ndyn*ndyn*ndyn))
  allocate(P2sm_temp_mpi(ksub_mpi*nkc*ndyn*ndyn*ndyn))
  ...
  allocate(P1sm_tempmpi(num_k_mpi2*nkc*ndyn*ndyn*ndyn))
  allocate(P2sm_tempmpi(num_k_mpi2*nkc*ndyn*ndyn*ndyn))

  allocate(p1sm_mpi(num_k_mpi2,nkc,ndyn,ndyn,ndyn))
  allocate(p2sm_mpi(num_k_mpi2,nkc,ndyn,ndyn,ndyn))
        ...
        do n2_mpi=1,nkc
           do l2_mpi=1,ndyn
             do l3_mpi=1,ndyn
                ...
                smpi = smpi +1
                P1sm_temp_mpi(smpi) = P1smpi(n1_mpi,n2_mpi,l1_mpi,l2_mpi,l3_mpi)
                P2sm_temp_mpi(smpi) = P2smpi(n1_mpi,n2_mpi,l1_mpi,l2_mpi,l3_mpi)
  ...
  ncount1_mpi=size(P1smpi)
  ncount2_mpi=size(P2smpi)

  tetttst = size(P1sm_tempmpi)

  !write(*,*)'rank_mpi,ksub_mpi,num_k_mpi2,size(P1smpi),size(P1sm_temp_mpi),size(P1sm_tempmpi)',rank_mpi,ksub_mpi,num_k_mpi2,size(P1smpi),size(P1sm_temp_mpi),size(P1sm_tempmpi)
 
!  write(*,*)'rank_mpi,ncount1_mpi,ncount2_mpi,tetttst',rank_mpi,ncount1_mpi,ncount2_mpi,tetttst 

 ! call MPI_Allgather(P1sm_temp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,P1sm_tempmpi,ncount1_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi) 
 ! call MPI_Allgather(P2sm_temp_mpi,ncount2_mpi,MPI_DOUBLE_PRECISION,P2sm_tempmpi,ncount2_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
  ...
  call MPI_Allgatherv(P2sm_temp_mpi,ncount2_mpi,MPI_DOUBLE_PRECISION,P2sm_tempmpi,aksub_mpi,displs_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)  


   !call MPI_Allgatherv(A, sendcount, MPI_INTEGER, AA, sendcounts, displs, MPI_INTEGER, MPI_COMM_WORLD, ierr)

!stop
!tsts=9356
!open(tsts,file='P1_test.dat',status='unknown')

!***    n_mpi=1
!***    do k_mpi=1,num_k_mpi
!***        p1sm_mpi(k_mpi,:,:,:,:)=reshape(P1sm_tempmpi(n_mpi:n_mpi+values1_mpi-1),shape(p1sm_mpi(k_mpi,:,:,:,:)))
!***        n_mpi=n_mpi+values1_mpi
!***        write(*,*) rank_mpi, k_mpi, num_k_mpi, values1_mpi, n_mpi
        !write(tsts,*)
!        if (n_mpi > size(p1sm_tempmpi)) exit
!***    enddo
!***   print *, 'MPI rank ', rank_mpi, 'finished expanding p1sm_mpi '

!close(tsts)
!stop
!***   n_mpi=1
!***   do k_mpi=1,num_k_mpi
!***       p2sm_mpi(k_mpi,:,:,:,:)=reshape(P2sm_tempmpi(n_mpi:n_mpi+values2_mpi-1),shape(p2sm_mpi(k_mpi,:,:,:,:)))
!***       n_mpi=n_mpi+values2_mpi
 !      if (n_mpi > size(p2sm_tempmpi)) exit
!***   enddo
!***   print *, 'MPI rank ', rank_mpi, 'finished expanding p2sm_mpi '
    ...
    do l1_mpi=1,ndyn
      do n2_mpi=1,nkc
        do l2_mpi=1,ndyn
           do l3_mpi=1,ndyn
              smpi=smpi + 1
              p1sm_mpi(n1_mpi,n2_mpi,l1_mpi,l2_mpi,l3_mpi) = P1sm_tempmpi(smpi) 
              p2sm_mpi(n1_mpi,n2_mpi,l1_mpi,l2_mpi,l3_mpi) = P2sm_tempmpi(smpi)
           enddo
         enddo
       enddo
    enddo
...
deallocate(v33s8,P1sm_temp_mpi,P1sm_tempmpi,P2sm_temp_mpi,P2sm_tempmpi,P1smpi,P2smpi,displs_mpi,aksub_mpi)

!writng p1sm_mpi,p2sm_mpi
!***  tto = 1336

!***  write(filename_mpi,fmt="(a,i3.3,a)") "p1sm_mpi..dat"
!***  write(filename_mpii,fmt="(a,i3.3,a)") "p2sm_mpi..dat"
!***  open(tto,file=filename_mpi,status='replace')
!***  open(tto+1,file=filename_mpii,status='replace')
...
!***               write(tto,*) n1_mpi, n2_mpi, l1_mpi, l2_mpi, l3_mpi,p1sm_mpi(n1_mpi,n2_mpi,l1_mpi,l2_mpi,l3_mpi)
!***               write(tto+1,*)  n1_mpi, n2_mpi, l1_mpi, l2_mpi, l3_mpi,p2sm_mpi(n1_mpi,n2_mpi,l1_mpi,l2_mpi,l3_mpi)
!***             enddo
!***           enddo
!***        enddo
!***     enddo
...
!***call allocate_iter (nibz,nkc,ndyn)    ! allocate memory
  ...
  call calculate_tauRTA_mpi2(ndyn,tempk)        
  endif
...
! ksub_mpi = (ksubset(2) - ksubset(1)) + 1
! Allocate array to gather sendcounts from all processes
...
! call MPI_Allgatherv(P1sm_temp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,P1sm_tempmpi,aksub_mpi,displs_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
  ...
  deallocate(CM_temp_mpi,Collision_Matrixmpi,P1,P2,cdispls_mpi,caksub_mpi)
  allocate(Collision_Matrix(num_k_mpi2*ndyn*3,nibz*ndyn*3))
...
! Gather sendcounts from all processes
  call MPI_Allgather(ksubmpiv, 1, MPI_INTEGER, aksub_mpi, 1, MPI_INTEGER, MPI_COMM_WORLD, ierr)
...
!!!write(lwse,*)tempk,l1_mpi,n1_mpi,kbss,dk_bs(n1_mpi),omega0(l1_mpi),ise_tempmpi(smpi)/2,ise_tempmpi(smpi),tse_tempmpi(smpi),1/tse_tempmpi(smpi)      
!write(lwse,*)tempk,l1_mpi,n1_mpi,kbss,dk_bs(n1_mpi),omega0(l1_mpi),ise_mpi(n1_mpi,l1_mpi)/2,ise_mpi(n1_mpi,l1_mpi),tse_mpi(n1_mpi,l1_mpi),1/tse_mpi(n1_mpi,l1_mpi) 
!write(*,*)l1_mpi,n1_mpi,ise_mpi(n1_mpi,l1_mpi)
    enddo
 !write(lwse2,*)tempk,dk_bs(nkbss),(omega0(la),la=1,ndyn),((1/tse(la)),la=1,ndyn) 
   write(lwse2,*)tempk,dk_bs(n1_mpi),(omega0(l1_mpi),l1_mpi=1,ndyn),((1/tse_mpi(n1_mpi,l1_mpi)),l1_mpi=1,ndyn)
       !!! write(lwse2,*)tempk,dk_bs(n1_mpi),(omega0(l1_mpi),l1_mpi=1,ndyn),((1/tse_mpi(l1_mpi,smpi)),l1_mpi=1,ndyn)
  enddo
...
!----------The anharmonic V3 matrix calculation in IBZ-------------------------------------
  nv3 = nkc*nibz*ndyn**3
!-------------------------------------------------------
elseif ( job .eq. 6 ) then   ! v33 in IBZ split...
!-------------------------------------------------------
write(ulog,*) 'entering cal v33 in IBZ split...'
write(*,*) 'entering cal v33 in IBZ split...'
write(ulog,*) 'job', job
...
write(utimes,'(a,f12.4)')'V33 calculation start,                TIME IS', cputim
        ...
        allocate(v33s8((ksubset(2)-ksubset(1)+1),nkc,ndyn,ndyn,ndyn))
        !K1     call calculate_w3_fbz_split_sq(ksubset,nv3_split,ndyn,nkc,kpc,eigenval,eigenvec)
        call calculate_w3_ibz_split_sq (ksubset,nv3_split,ndyn,nkc,kpc,eigenval,eigenvec,nv3o)
        write(*,*)'after v3'
        call cpu_time(cputim)  !date_and_time(time=tim)
        write(utimes,'(a,f12.4)')' After v33 calculation,                TIME IS ',cputim
        ...
        write(ulog,*) 'entering cal v33*delta...'
        write(*,*) 'entering cal v33*delta...'
        ! K1 changed to IBZ        call calculate_v3sq_delta (ksubset,ndyn,nkc,kpc)  
...
!-------------------------------------------------------
if(job .eq. 8) then  ! Direct noniterative method-svd 
!-------------------------------------------------------
...
!---------------------------------------------------------------
elseif(job .eq. 9) then   ! Direct noniterative method-split-svd
!---------------------------------------------------------------
...
!----------------------------------------------------------------------------
elseif (job .eq.10) then  !reading collision Matrix_split calculated by job=7  
!----------------------------------------------------------------------------
...
!-------------------------------------------------------
elseif(job .eq. 11) then  ! Direct noniterative method-cg
!-------------------------------------------------------
write(ulog,*) 'Start direct approach using cg method'
write(*,*) 'Start direct approach using cg method'
write(ulog,*) 'job', job
...
!------------------------------------------------------
elseif (job.eq.12) then  ! iterative solution here
!------------------------------------------------------
