...
integer          :: unit_numpi
integer          :: CM_1D, DS_1D
integer          :: maxksubset2,maxksubset1, maxksubset,sxyz_mpi,iksubmpiv
real(8), allocatable :: F2_tempi(:),F2temp(:)
integer, allocatable :: iaksub_mpi(:), idispls_mpi(:)

integer, allocatable :: aksub_mpi1(:), displs_mpi1(:)
integer, allocatable :: aksub_mpi2(:), displs_mpi2(:)
integer, allocatable :: aksub_mpir(:), displs_mpir(:)
complex(8), allocatable :: chi_temp_mpi(:),chi_tempmpi(:)
real(8), allocatable :: isetemp_mpi(:),rsetemp_mpi(:),ise_temmpi(:),rse_temmpi(:)
integer              :: ksubmpiv1,ksubmpiv2,ksubmpivr,ncountr_mpi
real(8), allocatable ::v3sqtemp_mpi(:),v3sq_temmpi(:)
real(8)              :: k_es2(3),v3sqval
integer              :: n_es2,smpi2,kem_shift
real(8), allocatable :: isetet_em(:,:)
!===================================
...
!call get_freq(k_es,ndyn,eigenval_es,eigenvec_es)
!call dielectric(ndyn,eigenval_es,eigenvec_es,wmesh,om,epsilond)
!stop


! eigenval = sqrt(abs(eigenval))*cnst

! k_es=0
!  k_es(3)=0.0001

!  call get_freq(k_es,ndyn,eigenval_es,eigenvec_es)
! eigenval_es = sqrt(abs(eigenval_es))*cnst

!  allocate(v3sq(1,nkc,ndyn,ndyn,ndyn))
!  k_es=0
!  k_es(3)=0.0001
  ! write(*,*)'before stopppppppppppp'
  !stop 
 ! call calculate_v35_selftetra_qzero(k_es,ndyn,nibz,eigenval_es,eigenvec_es,nkc,eigenval,eigenvec)
 ...
 ! call dielectric_setetra(ndyn,eigenval_es,eigenvec_es,wmesh,om,temp,epsilond)
...
!------------------------------------------------------
elseif (job.eq.3) then  ! iterative solution using MPI
!------------------------------------------------------
  write(ulog,*) 'entering cal v33 in IBZ split...'
  write(*,*) 'entering cal v33 in IBZ split...'
  write(ulog,*) 'job', job
...
write(utimes,'(a,f12.4)')' start iterative approach,                 TIME IS',cputim
        ...
        allocate(F2(nibz,ndyn,3),F_RTA(nibz,ndyn,3))
        F_RTA=FFF

  ksub_mpi = (ksubset(2) - ksubset(1)) + 1

  allocate(F2mpi((ksubset(2)-ksubset(1)+1),ndyn,3))
  allocate(F2_tempi((ksubset(2)-ksubset(1)+1)*ndyn*3))

  ! Allocate array to gather ksubs from all processes
  allocate(iaksub_mpi(nprocs_mpi))
  allocate(idispls_mpi(nprocs_mpi))

  iksubmpiv=ksub_mpi*ndyn*3

! Gather ksubs from all processes
  call MPI_Allgather(iksubmpiv, 1, MPI_INTEGER, iaksub_mpi, 1, MPI_INTEGER, MPI_COMM_WORLD, ierr_mpi)

  idispls_mpi(1) = 0
  do i = 2, nprocs_mpi
     idispls_mpi(i) = idispls_mpi(i-1) + iaksub_mpi(i-1)
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
  end do

  allocate(F2temp(num_k_mpi*ndyn*3))
  ncount1_mpi=size(F2mpi)
...
!           call update(ndyn)
  ...
  do n1_mpi=1,ksub_mpi
     do l1_mpi=1,ndyn
        ...
        smpi=smpi+1
        F2_tempi(smpi) = F2mpi(n1_mpi,l1_mpi,sxyz_mpi)
  ...
  !call MPI_Allreduce(F2_tempi, F2_temmpi, size(F2_tempi), MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr_mpi)
...
! start_timempi = MPI_Wtime()
  ...
  call MPI_Allgatherv(F2_tempi,ncount1_mpi,MPI_DOUBLE_PRECISION,F2temp,iaksub_mpi,idispls_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)

! end_timempi = MPI_Wtime()
! elapsed_timempi = end_timempi - start_timempi
! write(unit_numpi, '(A, F6.2)') "Time after subroutine-gather: ", elapsed_timempi
! flush(unit=unit_numpi)  ! Ensure the data is written to the file immediately
 ...
 F2=0
  ...
  do n1_mpi=1,num_k_mpi
     do l1_mpi=1,ndyn
        ...
        smpi=smpi+1
        F2(n1_mpi,l1_mpi,sxyz_mpi) = F2temp(smpi)
        enddo
     enddo
  enddo
...
!-------------------emissivity using self-energy calculation-MPI-----------------------------------
  elseif ( job .eq. 50 ) then
!---------------------------------------
  write(*,*)'emissivity-self-energy calculation start'
  write(ulog,*) 'emissivity-self-energy calculation start'
  write(ulog,*) 'job', job

  call cpu_time(cputim)  !date_and_time(time=tim)
  write(utimes,'(a,f12.4)')' emissivity-self-energy calculation,               TIME IS',cputim

  open(3451,file='chis_realse.dat')
  open(3461,file='chis_imagse.dat')
  open(3471,file='selfenergy.dat')
  open(3481,file='opticalconstants.dat')
  write(3451,*)'# omega(1/cm) , chi_xx , chi_xy , chi_xz  , chi_yx  ,  chi_yy  ,  chi_yz  , chi_zx , chi_zy , chi_zz , R, E'
...
! ksubset(1) and ksubset(2) are needed for the loops in the follwoing subroutines (calculate_w3_ibz_split_sq,calculate_FGR_mpi)
  ...
  !allocate(isempi((ksubset(2)-ksubset(1)+1),ndyn))
  !allocate(tsempi((ksubset(2)-ksubset(1)+1),ndyn))
  write(*,*)'hereeeeeeeeeeeeeeeee'
  
  !allocate(F2mpi((ksubset(2)-ksubset(1)+1),ndyn-3))

  allocate(isempi_em((ksubset(2)-ksubset(1)+1),(ndyn-3)))
  allocate(rsempi_em((ksubset(2)-ksubset(1)+1),(ndyn-3)))
  allocate(reflectivitympi(ksubset(2)-ksubset(1)+1))
  allocate(chimpi((ksubset(2)-ksubset(1)+1),3,3))

 allocate(nmpi(ksubset(2)-ksubset(1)+1))
 allocate(kmpi(ksubset(2)-ksubset(1)+1))
 allocate(reflectivitympink(ksubset(2)-ksubset(1)+1))
...
! enddo
  ...
  call dielectric_sempi(ksubset,ndyn,eigenval_es,eigenvec_es,wmesh,om,temp,epsilond)


  ksub_mpi = (ksubset(2) - ksubset(1)) + 1
! Allocate array to gather ksubsets from all processes
  allocate(aksub_mpi1(nprocs_mpi))
  allocate(displs_mpi1(nprocs_mpi))
  
  allocate(aksub_mpi2(nprocs_mpi))
  allocate(displs_mpi2(nprocs_mpi))
  
  allocate(aksub_mpir(nprocs_mpi))
  allocate(displs_mpir(nprocs_mpi))


  ksubmpiv1=ksub_mpi*(ndyn-3)
  ksubmpiv2=ksub_mpi*3*3
  ksubmpivr=ksub_mpi

  
! Gather ksubs from all processes
  call MPI_Allgather(ksubmpiv1, 1, MPI_INTEGER, aksub_mpi1, 1, MPI_INTEGER, MPI_COMM_WORLD, ierr)
  call MPI_Allgather(ksubmpiv2, 1, MPI_INTEGER, aksub_mpi2, 1, MPI_INTEGER, MPI_COMM_WORLD, ierr)
  call MPI_Allgather(ksubmpivr, 1, MPI_INTEGER, aksub_mpir, 1, MPI_INTEGER, MPI_COMM_WORLD, ierr)



  displs_mpi1(1) = 0
  do i = 2, nprocs_mpi
     displs_mpi1(i) = displs_mpi1(i-1) + aksub_mpi1(i-1)
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
  end do

  displs_mpi2(1) = 0
  do i = 2, nprocs_mpi
     displs_mpi2(i) = displs_mpi2(i-1) + aksub_mpi2(i-1)
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
  end do

  displs_mpir(1) = 0
  do i = 2, nprocs_mpi
     displs_mpir(i) = displs_mpir(i-1) + aksub_mpir(i-1)
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
  end do
...
! write(*,*) displs_mpi,aksub_mpi
 ...
 ! allocate(tse_temp_mpi(ksub_mpi*ndyn))
  ...
  allocate(isetemp_mpi(ksub_mpi*(ndyn-3)))
  allocate(rsetemp_mpi(ksub_mpi*(ndyn-3)))
  allocate(chi_temp_mpi(ksub_mpi*3*3))  
...
! write(*,*)num_k_mpi2,num_k_mpi,local_num_k_mpi, nprocs_mpi, mod(num_k_mpi,nprocs_mpi)
 ...
 ! allocate(tse_tempmpi(num_k_mpi2*ndyn))
  ...
  !allocate(tse_mpi(num_k_mpi2,ndyn))
   ...
   allocate(ise_temmpi(num_k_mpi2*(ndyn-3)))
   allocate(rse_temmpi(num_k_mpi2*(ndyn-3)))
   allocate(ise_emmpi(num_k_mpi2,ndyn-3))
   allocate(rse_emmpi(num_k_mpi2,ndyn-3))
   allocate(reflectivity_emmpi(num_k_mpi2))
   allocate(chi_tempmpi(num_k_mpi2*3*3))
   allocate(chi_emmpi(num_k_mpi2,3,3))

  allocate(n_emmpi(num_k_mpi2))
  allocate(k_emmpi(num_k_mpi2))
  allocate(reflectivity_emmpink(num_k_mpi2))
...
!  tse_temp_mpi =0
  ...
  do n1_mpi=1,ksub_mpi       !ksub_size2=ksubset(2)-ksubset(1)+1
     ...
     do l3_mpi=1,3
        ...
        !ise_temp_mpi(smpi) = isempi(n1_mpi,l3_mpi)
        !tse_temp_mpi(smpi) = tsempi(n1_mpi,l3_mpi)
        chi_temp_mpi(smpi) = chimpi(n1_mpi,l2_mpi,l3_mpi)
    enddo
    enddo
 write(*,*)'before-allgatherv',rank_mpi, n1_mpi,aimag(chimpi(n1_mpi,:,:))  
 write(*,*)'before-allgatherv',rank_mpi, n1_mpi, real(chimpi(n1_mpi,:,:))
...
enddo
  ...
  do n1_mpi=1,ksub_mpi
     do l2_mpi=4, ndyn
        ...
        smpi = smpi +1
        isetemp_mpi(smpi) = isempi_em(n1_mpi,l2_mpi-3) 
        rsetemp_mpi(smpi) = rsempi_em(n1_mpi,l2_mpi-3)
        write(*,*)'before-allgatherv',rank_mpi,n1_mpi,l2_mpi,isempi_em(n1_mpi,l2_mpi-3)
        write(*,*)'before-allgatherv',rank_mpi,n1_mpi,l2_mpi,rsempi_em(n1_mpi,l2_mpi-3)
     enddo
   enddo 
 ...
 !stop
   ncount1_mpi=size(isempi_em)
   ncountr_mpi=size(reflectivitympi)
   ncount2_mpi=size(chimpi)

!  call MPI_Allgatherv(ise_temp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,ise_tempmpi,aksub_mpi,displs_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
!  call MPI_Allgatherv(tse_temp_mpi,ncount2_mpi,MPI_DOUBLE_PRECISION,tse_tempmpi,aksub_mpi,displs_mpi,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)

   call MPI_Allgatherv(isetemp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,ise_temmpi,aksub_mpi1,displs_mpi1,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
   call MPI_Allgatherv(rsetemp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,rse_temmpi,aksub_mpi1,displs_mpi1,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
   call MPI_Allgatherv(reflectivitympi,ncountr_mpi,MPI_DOUBLE_PRECISION,reflectivity_emmpi,aksub_mpir,displs_mpir,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
   call MPI_Allgatherv(chi_temp_mpi,ncount2_mpi,MPI_DOUBLE_COMPLEX,chi_tempmpi,aksub_mpi2,displs_mpi2,MPI_DOUBLE_COMPLEX,MPI_COMM_WORLD,ierr_mpi)

   call MPI_Allgatherv(nmpi,ncountr_mpi,MPI_DOUBLE_PRECISION,n_emmpi,aksub_mpir,displs_mpir,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
   call MPI_Allgatherv(kmpi,ncountr_mpi,MPI_DOUBLE_PRECISION,k_emmpi,aksub_mpir,displs_mpir,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
   call MPI_Allgatherv(reflectivitympink,ncountr_mpi,MPI_DOUBLE_PRECISION,reflectivity_emmpink,aksub_mpir,displs_mpir,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
...
chi_emmpi=0
  smpi=0
  do n1_mpi=1,num_k_mpi2 !ksub_mpi       !ksub_size2=ksubset(2)-ksubset(1)+1
     do l2_mpi=1,3
     do l3_mpi=1,3
        ...
        !ise_temp_mpi(smpi) = isempi(n1_mpi,l3_mpi)
        !tse_temp_mpi(smpi) = tsempi(n1_mpi,l3_mpi)
       chi_emmpi(n1_mpi,l2_mpi,l3_mpi) = chi_tempmpi(smpi) 
      !write(*,*)'outside', n1_mpi, l2_mpi, l3_mpi, smpi, chi_tempmpi(smpi)  
     enddo
    enddo
    write(*,*)'outside1',om(n1_mpi), aimag(chi_emmpi(n1_mpi,:,:))
    write(*,*)'outside',om(n1_mpi), real(chi_emmpi(n1_mpi,:,:))  
enddo
...
!stop
  ...
  rse_emmpi=0
  smpi=0
  do n1_mpi=1,num_k_mpi2
     do l2_mpi=4, ndyn

        smpi = smpi +1
        ise_emmpi(n1_mpi,l2_mpi-3) = ise_temmpi(smpi)
        rse_emmpi(n1_mpi,l2_mpi-3) = rse_temmpi(smpi)
        !write(*,*)'before-allgatherv',rank_mpi,n1_mpi,l2_mpi,isempi_em(n1_mpi,l2_mpi-3)
        !write(*,*)'before-allgatherv',rank_mpi,n1_mpi,l2_mpi,rsempi_em(n1_mpi,l2_mpi-3)
     enddo
   enddo
...
! write(lwse2,*)tempk,dk_bs(nkbss),(omega0(la),la=1,ndyn),((1/tse(la)),la=1,ndyn)
 ! write(lwse2,*)tempk,dk_bs(n1_mpi),(omega0(l1_mpi),l1_mpi=1,ndyn),((1/tse_mpi(n1_mpi,l1_mpi)),l1_mpi=1,ndyn)
   ...
   !  write(*,*)'i,om,emissivity',n1_mpi,om(n1_mpi),real(chi_emmpi(n1_mpi,:,:)),reflectivity_emmpi(n1_mpi), 1-reflectivity_emmpi(n1_mpi)
     write(3451,50)om(n1_mpi),real(chi_emmpi(n1_mpi,:,:)),reflectivity_emmpi(n1_mpi), 1-reflectivity_emmpi(n1_mpi)
     write(3461,50)om(n1_mpi),aimag(chi_emmpi(n1_mpi,:,:))
     write(3481,50)om(n1_mpi),n_emmpi(n1_mpi),k_emmpi(n1_mpi),reflectivity_emmpink(n1_mpi), 1-reflectivity_emmpink(n1_mpi) 
! enddo
   do l2_mpi=4,ndyn

    write(3471,50) om(n1_mpi), l2_mpi, ise_emmpi(n1_mpi,l2_mpi-3),rse_emmpi(n1_mpi,l2_mpi-3)
   enddo
  enddo
...
50 format(999(1x,g11.4))

  call cpu_time(cputim)  !date_and_time(time=tim)
  write(utimes,'(a,f12.4)')' emissivity-Self-energy calculation done,     TIME IS',cputim
  write(ulog,*) ' emissivity-Self-energy calculation finished here '
  write(*,*)' emissivity-Self-energy calculation done'
call MPI_Finalize(ierr_mpi)
!----------------------------------------------------------------------------------------------

!-------------------emissivity using self-energy calculation-MPI-----------------------------------
  elseif ( job .eq. 51 ) then
!---------------------------------------
  write(*,*)'emissivity-self-energy calculation start'
  write(ulog,*) 'emissivity-self-energy calculation start'
  write(ulog,*) 'job', job
  ...
  open(3461,file='v3sq.dat')
  open(3463,file='v3sq2.dat')  
...
! ksubset(1) and ksubset(2) are needed for the loops in the follwoing subroutines (calculate_w3_ibz_split_sq,calculate_FGR_mpi)
  ...
  allocate(v3sqmpi_em(1,(ksubset(2)-ksubset(1)+1),ndyn,ndyn,ndyn))

  
  call calculate_v35_selftetra_qzerompi(ksubset,k_es,ndyn,nibz,eigenval_es,eigenvec_es,nkc,eigenval,eigenvec)
...
! Allocate array to gather ksubsets from all processes
  ...
  ksubmpiv1=ksub_mpi*ndyn*ndyn*ndyn
...
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
  ...
  allocate(v3sqtemp_mpi(ksub_mpi*ndyn*ndyn*ndyn))
  ...
  if (mod(num_k_mpi,nprocs_mpi).ne.0) then
   ...
   allocate(v3sq_temmpi(num_k_mpi2*ndyn*ndyn*ndyn))
  ...
  do n1_mpi=1,ksub_mpi       !ksub_size2=ksubset(2)-ksubset(1)+1
     ...
     do l3_mpi=1,ndyn
        ...
        smpi = smpi +1
        v3sqtemp_mpi(smpi) = v3sqmpi_em(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)
       write(*,*)'rank',rank_mpi,1,n1_mpi,l1_mpi,l2_mpi,l3_mpi, v3sqmpi_em(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)
...
enddo
   ...
   ncount1_mpi=size(v3sqmpi_em)


   call MPI_Allgatherv(v3sqtemp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,v3sq_temmpi,aksub_mpi1,displs_mpi1,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
  
allocate(v3sq(1,num_k_mpi2,ndyn,ndyn,ndyn))
!stop
...
v3sq=0
  ...
  do n1_mpi=1,num_k_mpi2 !ksub_mpi       !ksub_size2=ksubset(2)-ksubset(1)+1
    k_es2 = kpc(:,n1_mpi)
    call get_k_info(k_es2-shft,NC,n_es2,i2,j2,k2,g1,g2,g3,inside)
     do l1_mpi=1,ndyn
     do l2_mpi=1,ndyn
     do l3_mpi=1,ndyn
        ...
        !ise_temp_mpi(smpi) = isempi(n1_mpi,l3_mpi)
        !tse_temp_mpi(smpi) = tsempi(n1_mpi,l3_mpi)
       v3sq(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi) = v3sq_temmpi(smpi) 
      write(3461,*)n1_mpi,1,n_es2,l1_mpi, l2_mpi, l3_mpi, v3sq(1,n_es2,l1_mpi,l2_mpi,l3_mpi),v3sq(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)  
      write(3463,*)n_es2,v3sq(1,n_es2,l1_mpi,l2_mpi,l3_mpi)
      write(*,*)'enddd', n1_mpi,l1_mpi, l2_mpi, l3_mpi, v3sq(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)
      enddo
    enddo
enddo
enddo
...
!---------------------------------------
  ...
  call cpu_time(cputim)  !date_and_time(time=tim)
  write(utimes,'(a,f12.4)')' emissivity-self-energy calculation,               TIME IS',cputim
write(*,*)'inside-52222222222222222222222'
allocate(v3sq(1,nkc,ndyn,ndyn,ndyn))
  open(3471,file='v3sq2.dat', status='old')
v3sq=0
  do n1_mpi=1,nkc 
  !  k_es2 = kpc(:,n1_mpi)
  !  call get_k_info(k_es2-shft,NC,n_es2,i2,j2,k2,g1,g2,g3,inside)
     ...
     do l3_mpi=1,ndyn
          ...
          read(3471,*)n_es2, v3sqval
         v3sq(1,n_es2,l1_mpi,l2_mpi,l3_mpi)=v3sqval  
         write(*,*)'readdddd', n1_mpi,l1_mpi, l2_mpi, l3_mpi, v3sq(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)
      enddo
    enddo
enddo
enddo
...
! ksubset(1) and ksubset(2) are needed for the loops in the follwoing subroutines (calculate_w3_ibz_split_sq,calculate_FGR_mpi)
  ...
  allocate(isetet_em((ksubset(2)-ksubset(1)+1),ndyn-3))
...
!write(*,*)'i---------------',i
       ...
       call function_self_tetra(1,k_es,l1_mpi,om(n1_mpi+kem_shift),temp,nselfs,uselfs)

       isetet_em(n1_mpi,smpi) = aimag(nselfs+uselfs)

       write(*,*)'i,ise,rse',n1_mpi,smpi,om(n1_mpi),l1_mpi, isetet_em(n1_mpi,smpi)!, rsempi_em(i,s)
...
!    write(*,*)'displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i),',i, displs_mpi(i-1),displs_mpi(i),aksub_mpi(i-1),aksub_mpi(i)
 ...
 !isetemp_mpi(:),rsetemp_mpi(:),ise_temmpi(:) 
  allocate(isetemp_mpi(ksub_mpi*(ndyn-3)))
  ...
  do n1_mpi=1,ksub_mpi       
     do l1_mpi=4,ndyn
        ...
        smpi = l1_mpi-3
        smpi2 = smpi2+1
        isetemp_mpi(smpi2) = isetet_em(n1_mpi,smpi)
       !write(*,*)'rank',rank_mpi,1,n1_mpi,l1_mpi,l2_mpi,l3_mpi, v3sqmpi_em(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)

    enddo
  enddo

deallocate(isetet_em)

   ncount1_mpi=size(isetet_em)


   call MPI_Allgatherv(isetemp_mpi,ncount1_mpi,MPI_DOUBLE_PRECISION,ise_temmpi,aksub_mpi1,displs_mpi1,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,ierr_mpi)
  
allocate(isetet_em(num_k_mpi2,ndyn-3))
!stop
...
isetet_em=0
  ...
  do n1_mpi=1,num_k_mpi2 !ksub_mpi       !ksub_size2=ksubset(2)-ksubset(1)+1
     ...
     do l1_mpi=4,ndyn
        ...
        !ise_temp_mpi(smpi) = isempi(n1_mpi,l3_mpi)
        !tse_temp_mpi(smpi) = tsempi(n1_mpi,l3_mpi)
       isetet_em(n1_mpi,smpi) = ise_temmpi(smpi2) 
      !write(3461,*)n1_mpi,1,n_es2,l1_mpi, l2_mpi, l3_mpi, v3sq(1,n_es2,l1_mpi,l2_mpi,l3_mpi),v3sq(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)  
      !write(*,*)'enddd', n1_mpi,l1_mpi, l2_mpi, l3_mpi, v3sq(1,n1_mpi,l1_mpi,l2_mpi,l3_mpi)
      enddo
   enddo
...
!call dielectric_setetrampirzero(n,eival,eivec,mesh,om,tm,ise_em,epsilon0)
call dielectric_setetrampirzero(ndyn,eigenval_es,eigenvec_es,wmesh,om,temp,isetet_em,epsilond)
endif
...
!-------------------------------------------------------
elseif ( job .eq. 4 ) then   ! MPI for very-large kpoints (with job=4)
!-------------------------------------------------------
  write(ulog,*) 'entering cal v33 in IBZ split...'
  write(*,*) 'entering cal v33 in IBZ split...'
  write(ulog,*) 'job', job
...
!----if CM is too big, this can be used as the second part to read CM and calculate kappa_direct-------------------
  elseif (job .eq. 5) then !(after running job=3)
!---------------------------------------------------
  write(ulog,*) 'Start direct approach using cg method'
  write(*,*) 'Start direct approach using cg method'
  write(ulog,*) 'job', job
...
!----------The anharmonic V3 matrix calculation in IBZ-------------------------------------
  nv3 = nkc*nibz*ndyn**3
!-------------------------------------------------------
  elseif ( job .eq. 9 ) then   ! v33 in IBZ split...
!-------------------------------------------------------
  write(ulog,*) 'entering cal v33 in IBZ split...'
  write(*,*) 'entering cal v33 in IBZ split...'
  write(ulog,*) 'job', job
...
!-------------------------------------------------------
  if(job .eq. 11) then  ! Direct noniterative method-svd 
!-------------------------------------------------------
...
!---------------------------------------------------------------
  elseif(job .eq. 12) then   ! Direct noniterative method-split-svd
!---------------------------------------------------------------
...
!----------------------------------------------------------------------------
  elseif (job .eq. 13) then  !reading collision Matrix_split calculated by job=11  
!----------------------------------------------------------------------------
...
!-------------------------------------------------------
  elseif(job .eq. 14) then  ! Direct noniterative method-cg
!-------------------------------------------------------
  write(ulog,*) 'Start direct approach using cg method'
  write(*,*) 'Start direct approach using cg method'
  write(ulog,*) 'job', job
...
!------------------------------------------------------
  elseif (job.eq.15) then  ! iterative solution here
!------------------------------------------------------
  write(ulog,*) 'iterative approach start'
  write(*,*) 'iterative approach start'
  write(ulog,*) 'job', job
