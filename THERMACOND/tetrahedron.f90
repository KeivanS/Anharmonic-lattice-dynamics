...
! assign eigenvalues to each corner of tetrahedra
 ...
 do j=1,nkp*6                 !iterate over all tetrahedra
    do k=1,4                  !iterate over four corners of tetrahedron
     ...
     !  tet(j)%p(k)%w=cnst*sqrt(eig(n))
!     write(*,*)'j,k,n,tet(j)%p(k)%w',j,k,n,tet(j)%p(k)%w,eig(n)
    enddo
 enddo

 end subroutine eigen_tet0
!-------------------------------------
 subroutine weight_tet0(nkp,energy)
...
! assign the eigenvalues to each tetrahedron
 call eigen_tet0(nk,arg)    

! assign weights to corners of all tetrahedrons
 call weight_tet0(nk,om) 
!write(*,*)'inside-tet-summmmmmmmmmmmmmmmmm'
...
!     write(*,*)'inside-tet-sum',k,func(k),array(k),term,tet(j)%p(l)%c,nk 
   enddo
 enddo
 res=sum(array)
