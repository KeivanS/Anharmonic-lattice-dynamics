...
subroutine allocate_tetra(num1_tet,num2_tet,num3_tet,num4_tet,num5_tet)
 implicit none

 integer num1_tet, num2_tet, num3_tet, num4_tet, num5_tet


 allocate(calct(num4_tet,num5_tet))
 allocate(doswt(num4_tet,num5_tet))
 allocate(tet((num1_tet)*(num2_tet)*(num3_tet)*6))
...
real(8), allocatable::xx2(:),sig(:),col_matrix(:,:),RHS2(:),sy_matrix(:,:),coll(:,:),F_MRHS(:),col_sy(:,:)
...
subroutine allocate_iter1 (nk,ndn)
allocate (F_RTA(nk,ndn,3),F1(nk,ndn,3), Fs1(nibz,ndn,3),F2(nk,ndn,3),F1_old(nk,ndn,3),Qvalue(nk,ndn),iselfenergy(nk,ndn),tau(nk,ndn,3),error(nk,ndn,3),diff(nk,ndn,3))
allocate (diff_kap(nk,ndn,3,3),kappa(ndn,3,3),kappa_k(nk,ndn,3,3),kappa_RTA(ndn,3,3),kappa_k_RTA(nk,ndn,3,3))    ! 3 for xyz
allocate (dist(nk,ndn),frequency(nk,ndn))
allocate (Qvalue_N(nk,ndn),Qvalue_U(nk,ndn),tauinv_N(nk,ndn),tauinv_U(nk,ndn),tauinv_tot(nk,ndn),tauinv_eff(nk,ndn,3))
...
!==============================================
module params
 real(8) tolerance,margin
 integer nconfigs,classical,ntemp,fdfiles,cal_cross,threemtrx,lamin,lamax
...
! integer, parameter:: ubs=50,udos=40,ueiv=60,ukap=70,uslf=110, &
!& ufc0=20,ualph=27,uvel=31,ufine=67,  &
!& fbz=93,ibzf=95,ibzc=94,ibs=96,ucors=65,uibzf=66,uv3=85,ugrun=86,urate=88
