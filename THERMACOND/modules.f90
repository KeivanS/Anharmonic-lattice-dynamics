...
integer ncol
real(8), allocatable:: F_RTA(:,:,:), F1(:,:,:), Fs1(:,:,:) ,F2(:,:,:), Qvalue(:,:), iselfenergy(:,:), tau(:,:,:), error(:,:,:),diff(:,:,:),F2mpi(:,:,:)
...
! eigenval_c for (nibz_coarse,kibz), eigenval_f for (nbz_fine,kibzf)
 ...
 real(8), allocatable :: eigenval_f(:,:),eigenval_bs(:,:),eigenval(:,:),eigenval_es(:)
! sqr of phonon freqs in eV/A/A/m0
 complex(8), allocatable :: eigenvec_f(:,:,:),eigenvec_bs(:,:,:),eigenvec(:,:,:),grun(:,:),grun_bs(:,:), eigenvec_es(:,:)

    contains

    subroutine allocate_eig(nb,ni,nk) ! nb for band, nk for coarse mesh in FBZ
    integer nb,nk,ni
      allocate( eigenval(nb,ni),eigenvec(nb,nb,ni),grun(nb,ni),veloc(3,nb,nk), eigenval_es(nb),eigenvec_es(nb,nb) )
...
!==========================================================
 module phi3
 complex(8), allocatable :: selfN(:,:,:) ,selfU(:,:,:),chimpi(:,:,:),chi_emmpi(:,:,:) !v3(:,:,:,:,:),
 real(8), allocatable :: v33sq(:),v33sq_5(:,:,:,:,:),v33s8(:,:,:,:,:),P1smpi(:,:,:,:,:),P2smpi(:,:,:,:,:),p1sm_mpi(:,:,:,:,:),p2sm_mpi(:,:,:,:,:),v3sq(:,:,:,:,:),v3sqmpi_em(:,:,:,:,:)
 real(8), allocatable ::isempi(:,:),tsempi(:,:),ise_mpi(:,:),tse_mpi(:,:),rsempi_em(:,:),isempi_em(:,:),reflectivitympi(:),ise_emmpi(:,:),rse_emmpi(:,:),reflectivity_emmpi(:) 
 real(8), allocatable ::nmpi(:),kmpi(:),reflectivitympink(:),reflectivity_emmpink(:),n_emmpi(:),k_emmpi(:)
...
! one fine grid in the irreducible FBZ for kappa/dos sums: kibz(3,nibz)
 ...
 real(8) shft(3),normald(3),deltak,kcutnbf,kcutnbc   ! kcutnb for nbrlst and interpolation
